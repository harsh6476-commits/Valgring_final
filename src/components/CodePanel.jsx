...
export default function CodePanel({ filename, text, language, label, showToast }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-lg border border-slate-800 bg-[#0f172a] my-2">
      {/* Top terminal bar */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-[#1e293b] border-b border-slate-700/60">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-xs text-slate-400 font-medium flex items-center gap-1.5">
        ...
        <button
          onClick={handleCopy}
          className={`
            text-[11px] font-mono font-semibold px-2.5 py-1 rounded-lg transition-all flex items-center gap-1.5 cursor-pointer
            ${copied
      ...
      {/* Code body with generous padding */}
      <div className="p-5 md:p-6 font-mono text-xs md:text-[13px] overflow-x-auto text-slate-200">
        {renderContent()}
      </div>

      {/* Optional sub-label footer */}
      {label && (
        <div className="px-4 py-2 border-t border-slate-800 bg-[#0b1120]">
